...
?>
   ...
   <table cellpadding="0" cellspacing="0" border="0" class="display" id="example" width="100%">
	...
	<tbody>
     <?php
      $list = "SELECT * FROM complain where lecturer='$user' order by cid desc";
			$result = $connector->query($list);
			if(mysql_num_rows($result) > 0)
				{
				while($data = mysql_fetch_assoc($result))
					...
					echo "<td><a href='response.php?id={$data['cid']}' onclick='$(this).modal({width:400, height:450}).open(); return false;'>Send response</a> <br>
   ...
   <h3> Chart</h3>
            ...
            //complains all
            $count_sqlall1 = "select count(cid) from complain where lecturer='$user'";
            $count_resall1 = $connector->query($count_sqlall1);
            $countall1 = @mysql_result($count_resall1, 0, "count(cid)");
            
            $count_sql = "select count(cid) from complain where category='result complaint' and  lecturer='$user'";
            ...
            $count_sql2 = "select count(cid) from complain where category='lecturer complaint' and lecturer='$user'";
