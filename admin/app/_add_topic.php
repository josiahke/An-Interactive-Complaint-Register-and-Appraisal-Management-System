...
?>
   ...
   <div id='content' >
	...
	<ul>
		...
		<li><a href="#tabs-2">Add New Topic</a></li>
	</ul>
	<!-- begin tab 1 -->   
	<div id="tabs-1">
   <table cellpadding="0" cellspacing="0" border="0" class="display" id="example" width="100%">
      ...
      $list = "SELECT * FROM topic order by tid desc";
			...
			if(mysql_num_rows($result) > 0)
				...
				while($data = mysql_fetch_assoc($result))
					...
					$query  = "SELECT * FROM user WHERE regno='$author'";
					$result2 = $connector->query($query);
					
						while($row = mysql_fetch_assoc($result2))
						{ //echo "<img src='{$row['picture']}' width='50' height='50'/> <br/>" ;
						if ($author!='admin'){$pic="../../app/".$row['picture'];} else {$pic=$row['picture'];}}
					...
					$query5  = "SELECT * FROM user WHERE regno='$author2'";
					$result5 = $connector->query($query5);
					
					while($row2 = mysql_fetch_assoc($result5))
						{ //echo "<img src='{$row['picture']}' width='50' height='50'/> <br/>" ;
						if ($author2!='admin'){$pic2="../../app/".$row2['picture'];} else {$pic2=$row2['picture'];}}		
    ...
    <div id="tabs-2">
    <form action="_save_topics.php" method="post" name="savetopics" >
	<label for="details">Write Topic:</label> 
	<textarea name="body" cols="50" rows="10" required title="Add your Topic"></textarea>
    <br />
	<input name="Save Topic" type="submit" value="Save Topic" /> 
	</form>
