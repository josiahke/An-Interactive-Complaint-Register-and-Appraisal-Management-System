<?php
   ...
   //random no
	 ...
	 //login query
	$qry = "SELECT * FROM user  WHERE regno='$jinamsee' AND password='$enc_mseekey' and cat='regular'";
	...
	if($result) {
		if(mysql_num_rows($result) > 0) {
			...
			session_register('cramis_sys');
			$_SESSION['cramis_sys']['jinamsee'] = $jinamsee;
			$_SESSION['cramis_sys']['mseerd'] =enc($rd);
			$_SESSION['cramis_in'] = true;
			$session = "1";
			session_write_close();
			header("location: index.php");
			exit();
